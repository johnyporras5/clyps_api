...
import {
  WorkerList,
  PaginatedWorkerListResult,
} from './interface/worker-list.interface';
...
export class CompanyWorkerService {
  ...
  async updateWorkerByUserId(
    ...
    if (!company) {
      throw new UnauthorizedException(
        'Solo Administradores pueden modificar trabajadores',
      );
    }
  ...
  async removeWorkerByUserId(
    ...
    if (!company) {
      throw new UnauthorizedException(
        'Solo Administradores pueden desactivar trabajadores',
      );
    }
  ...
  async getCompanyWorkersWithNameFilterPaginated(
    ...
    if (isActive !== undefined) {
      queryBuilder.andWhere('cw.is_active = :isActive', {
  ...
  private async paginateQueryBuilder<T>(
    ...
    if (isActive !== undefined) {
      countQueryBuilder.andWhere('cw.is_active = :isActive', {
        isActive: parseInt(isActive),
      });
    }
