...
export class AddServiceCategory1771460721072 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`service_category\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(145) NOT NULL, \`company_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `ALTER TABLE \`service\` ADD \`category_id\` int NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`service_category\` ADD CONSTRAINT \`FK_5015996724351d05c61ea6b2faf\` FOREIGN KEY (\`company_id\`) REFERENCES \`company\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`service_category\` DROP FOREIGN KEY \`FK_5015996724351d05c61ea6b2faf\``,
    );
    await queryRunner.query(
