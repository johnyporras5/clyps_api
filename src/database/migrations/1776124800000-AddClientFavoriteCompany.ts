...
export class AddClientFavoriteCompany1776124800000 implements MigrationInterface {
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`client_favorite_company\` DROP FOREIGN KEY \`FK_client_favorite_company_company\``);
        await queryRunner.query(`ALTER TABLE \`client_favorite_company\` DROP FOREIGN KEY \`FK_client_favorite_company_client\``);
        await queryRunner.query(`DROP INDEX \`IDX_client_favorite_company_company\` ON \`client_favorite_company\``);
        await queryRunner.query(`DROP INDEX \`IDX_client_favorite_company_client\` ON \`client_favorite_company\``);
        await queryRunner.query(`DROP INDEX \`UQ_client_favorite_company_client_company\` ON \`client_favorite_company\``);
