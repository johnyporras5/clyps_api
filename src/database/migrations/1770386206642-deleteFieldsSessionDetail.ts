...
export class DeleteFieldsSessionDetail1770386206642 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`session_detail\` DROP COLUMN \`description\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`session_detail\` DROP COLUMN \`description_ia\``,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`session_detail\` ADD \`description_ia\` text NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`session_detail\` ADD \`description\` text NULL`,
    );
  }
}
