  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!hasRole) {
      ...
      if (requiredRoleNames.length === 1) {
        throw new ForbiddenException(`Solo ${requiredRoleNames[0]}s pueden acceder a este recurso`);
      } else {
        throw new ForbiddenException(`Solo ${requiredRoleNames.join(' o ')} pueden acceder a este recurso`);
      }
    }
