...
export class SessionService {
  ...
  async updateDetailStatus(
    ...
    //     en sus servicios.
    if (parentSession && (parentSession.sessionStatus === 4 || parentSession.sessionStatus === 5 || parentSession.sessionStatus === 6)) {
      throw new BadRequestException(
 ...
 */
  private async updateSessionStatusBasedOnDetails(sessionId: number, cancelledBy?: string): Promise<{
    ...
    //     (ver paso 5.1).
    if (session.sessionStatus === 4 || session.sessionStatus === 5 || session.sessionStatus === 6) {
  ...
  async cancelSession(
    ...
    if (session.sessionStatus === 3 || session.sessionStatus === 4 || session.sessionStatus === 6) {
      throw new BadRequestException(
