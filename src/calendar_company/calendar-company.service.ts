...
export class CalendarCompanyService {
  ...
  async create(
    createCalendarCompanyDto: CreateCalendarCompanyDto,
  ): Promise<CalendarCompany> {
    const calendarCompany = this.calendarCompanyRepository.create(
      createCalendarCompanyDto,
    );
  ...
  async update(
    id: number,
    updateCalendarCompanyDto: UpdateCalendarCompanyDto,
  ): Promise<CalendarCompany> {
    const calendarCompany = await this.calendarCompanyRepository.findOne({
      where: { id },
    });
